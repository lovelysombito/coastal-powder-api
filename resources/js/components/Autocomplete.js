...
const Autocomplete = (props) => {
    ...
    const activeIndex = options.findIndex(option => option.toLowerCase().indexOf(value.toLowerCase()) > -1);
    ...
    const handleOnKeyDown = e => {
        if(e.keyCode == 40 && activeOptionIndex != (options.length-1)) {
            ...
            if(!e.shiftKey) {
                ...
                if(props.onChange) {
                    props.onChange(options[activeOptionIndex]);
                }
                if(props.onBlur) {
                    props.onBlur(e);
                }
    ...
    const handleOnChange = e => {
        ...
        if( val && val !== '') {
            ...
            const filteredOptions = origOptions.filter(option => option.toLowerCase().indexOf(val) > -1);
            setOptions(filteredOptions);
            const activeIndex = filteredOptions.findIndex(option => option.toLowerCase().indexOf(val.toLowerCase()) > -1);
    ...
    return (
            <React.Fragment>
                <input autoComplete={'off'} {...props} onFocus={handleOnFocus} onKeyDown={handleOnKeyDown} onBlur={handleonBlur} onChange={handleOnChange} value={value}  type="text" />
                {
                    optionsOpened && (
