...
class SyncDealJobs implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        Log::info("SyncDealJobs@handle - {$event->subscriptionType}", ["event"=> $event]);

        $integration = HubSpot::where(['platform' => 'HUBSPOT', 'platform_account_id'=>$event->portalId, 'integration_status'=>'Connected'])->first();
        if (!$integration) {
            Log::warning("SyncDealJobs@handle - HubSpot integration is not connected for ".$event->portalId, ["event"=> $event, "integration"=>$integration]);
            return;
        }

        if ($hubspotRetryTimestamp = Cache::get('hubspot-api-retry-timeout', null)) {
            Log::notice("SyncDealJobs@handle - HubSpot API rate limit activated, retrying in ".$hubspotRetryTimestamp - time()." seconds", ["jobId" => $this->uuid ?? 'job_id_doesnt_exist', "event"=> $event, "integration"=>$integration, "dealId"=>$this->event->objectId]);
        ...
        try {
            ...
            if (!$hubspotDeal) {
                Log::error("SyncDealJobs@handle - HubSpot deal not found for id ".$event->objectId, ["event"=> $event, "integration"=>$integration]);
                return;
            }
            ...
            if ($deal = Deal::where('hs_deal_id', $this->event->objectId)->first()) {
                ...
                $properties = HubSpot::castDealToArrayProperties($hubspotDeal, $hubspotCompanyName);
                $properties['hs_deal_id'] = $this->event->objectId;
                $deal = Deal::create($properties);
            }

            $items = $integration->getDealLineItemAssociations($this->event->objectId);
            ...
            if (!$lineItems) {
                Log::error("SyncDealJobs@handle - No line items found for deal ".$event->objectId, ["event"=> $event, "integration"=>$integration]);

                if ($deal->lineitems()) {
                    Log::info("SyncDealJobs@handle - Remove all related lineitems for ".$deal->deal_id, ["event"=> $event, "integration"=>$integration]);
                    $deal->lineitems()->delete();
                }

                if ($deal->jobs()) {
                    Log::info("SyncDealJobs@handle - Remove all related jobs for ".$deal->deal_id, ["event"=> $event, "integration"=>$integration]);
                    $deal->jobs()->delete();
                }
                return;
            }
            ...
            Log::info("SyncDealJobs@handle - Query unique colour combinations", ["event"=> $event, "integration"=>$integration, "job_combinations" => $uniqueColourMaterialCombinations]);
                ...
                foreach($colourCombination as $material => $materialCombination) {
                    foreach($materialCombination as $bay => $bayCombination) {
                        foreach ($bayCombination as $treatment => $uniqueJob) {
                            ...
                            if (!$job = Job::where(['colour' => $colour, 'material' => $material, 'powder_bay' => $bay, 'treatment' => $treatment, 'deal_id' => $deal->deal_id])->first()) {
                                ...
                                Log::info("SyncDealJobs@handle - Created job ".$job->job_id, ["event"=> $event, "integration"=>$integration]);
                            } else {

                                if ($job->chem_status) {
                                    unset($bayValues['chem_status']);
                                    unset($lineitemBayValues['chem_status']);
                                }
                                if ($job->treatment_status) {
                                    unset($bayValues['treatment_status']);
                                    unset($lineitemBayValues['treatment_status']);
                                }
                                if ($job->burn_status) {
                                    unset($bayValues['burn_status']);
                                    unset($lineitemBayValues['burn_status']);
                                }
                                if ($job->blast_status) {
                                    unset($bayValues['blast_status']);
                                    unset($lineitemBayValues['blast_status']);
                                }
                                if ($job->powder_status) {
                                    unset($bayValues['powder_status']);
                                    unset($lineitemBayValues['powder_status']);
                                }

                                $job->update(array_merge([
                                    'job_prefix' => $jobPrefix,
                                ], $bayValues));
                                Log::info("SyncDealJobs@handle - Updated job ".$job->job_id, ["event"=> $event, "integration"=>$integration]);
                            }
                            ...
                            foreach($uniqueJob as $lineitem) {
                                if ($existingLineItem = LineItems::where('hs_deal_lineitem_id', $lineitem['line_item_id'])->first()) {
                                    ...
                                    if ($currentJob->job_id === $job->job_id || ($currentJob->material === $material && $currentJob->colour === $colour && $currentJob->powder_bay === $bay && $currentJob->treatment === $treatment)) {
                                        ...
                                        Log::info("SyncDealJobs@handle - Updated line item ".$existingLineItem->line_item_id, ["event"=> $event, "integration"=>$integration]);
                                    } else {
                                        ...
                                        Log::info("SyncDealJobs@handle - Updated line item with ".$existingLineItem->line_item_id . " with new job id: ".$job->job_id, ["event"=> $event, "integration"=>$integration]);
                                    }
                                } else {
                                    $lineitemProperties = HubSpot::castLineItemToJobProperties($lineitem);
                                    $lineitemProperties['job_id'] = $job->job_id;
                                    ...
                                    Log::info("SyncDealJobs@handle - Created line item ".$newLineItem->line_item_id, ["event"=> $event, "integration"=>$integration]);
                                }
                            }
                        }
                    }
            ...
            foreach($jobs as $job) {
                if (count($job->lines) === 0) {
                    Log::info("SyncDealJobs@handle - Deleted job ".$job->job_id . " due to no associated line items", ["event"=> $event, "integration"=>$integration]);
                    $job->delete();
                }
            }
            ...
            if ($e->getCode() === 429) {
                $retryAfter = 10;
                Log::notice("SyncDealJobs@handle - HubSpot API rate limit exceeded, retrying in {$retryAfter} seconds", ["jobId" => $this->uuid ?? 'job_id_doesnt_exist', "event"=> $event, "integration"=>$integration, "dealId"=>$this->event->objectId]);
            ...
            Log::error("SyncDealJobs@handle - HubSpot API exception - ".$e->getMessage(), ["event"=> $event, "integration"=>$integration, "dealId"=>$this->event->objectId, "error" => ['message'=>$e->getMessage(), 'code'=>$e->getCode(), 'file'=>$e->getFile(), 'line'=>$e->getLine()],]);
            return;
        } catch (\Exception $e) {
            Log::error("SyncDealJobs@handle - Something has gone wrong: ".$e->getMessage(), [
